    ...
    var BJUI = {
        ...
        initLayout: function(ww) {
            ...
            var navbarWidth = $('body').data('bjui.navbar.width'),
                $header = $('#bjui-header'), 
                $toggle = $header.find('.bjui-navbar-toggle'), 
                $logo = $header.find('.bjui-navbar-logo'), 
                $navbar = $('#bjui-navbar-collapse'), 
                $nav = $navbar.find('.bjui-navbar-right');
...
+function ($) {
    ...
    $(function() {
        ...
        /* ajax加载提示 */
        BJUI.setRegional('progressmsg', '正在努力加载数据，请稍等...  如长时间加载，请刷新浏览器！')
    ...
    $.extend(String.prototype, {
        ...
        replacePlh: function($box) {
            ...
            return this.replace(/{\/?[^}]*}/g, function($1) {
                ...
                /******************************2017年5月3日17:24:28 zhilian start 多个匹配时传多个值**********************/
                var results = "";
                if($input&& $input.val()){
...
+function ($) {
    ...
    Slidebar.prototype.initHnav = function() {
        ...
        $array.each(function(i) {
            // zhilian faicon add 'fa-'
            var $t = $(this), panel, cls, bodycls, faicon = $t.data('faicon'), faiconClose = $t.data('faiconClose'), icon = faicon ? faicon : 'fa-dot-circle-o'
            ...
            // zhilian remove fa-
            if (icon) panel = panel.replaceAll('#icon#', '<i class="fa '+ icon +'"></i>')
    ...
    Dialog.prototype.open = function() {
        ...
        if (!(options.target || $(options.target).length) && !options.html) {
            ...
            if (!options.url) {
                ...
                /***************************** zhilian 2017年3月4日 start************************************************/
                if(event){
                    var obj = event.srcElement ? event.srcElement : event.target;
                    options.url = decodeURI(options.url).replacePlh($(obj).parents(".unitBox:first"))
                }else{
                    options.url = decodeURI(options.url).replacePlh()
                }
                /****************************** zhilian 2017年3月4日 end***********************************************/
                if (!options.url.isFinishedTm()) {
...
+function ($) {
    ...
    Bjuiajax.prototype.divcallback = function(json, $target, options) {
        ...
        if (options.reload && !forward) {//增加forward判断  zhilian
    ...
    // zhilian begin
	Bjuiajax.prototype.divListCallback = function(json) {
		var that = this;
		if (json.closeCurrent)
			that.$element.dialog('closeCurrent');
		var list = json.divInfoList.split(";");
		for(var i=0;i<list.length;i++){
			 var ele = list[i];
			 var eleArray = ele.split(":");
			 var divId = eleArray[0];
			 var urlPath = eleArray[1];
			 $('#' + divId).ajaxUrl({
				url : urlPath,
				loadingmask : true
			 });
...
+function ($) {
    ...
    Tablefixed.prototype.init = function() {
        ...
        if (this.$container.hasClass('tab-pane')) width = this.$container.parent().width() - 5;
        ...
        this.resizeCol();
        this.resizeGrid();
        if (height && !this.$fixed.closest('.tab-content').length) {
        	this.$fixed.height(height).addClass('fixedH');
        } else {
	    // zhilian.begin 默认不设height时自动计算高度
            var dialogHeight = this.$fixed.closest('.dialogShadow').height();
            if(dialogHeight){// dialogHeight 上的 tablefixed            	
            	var $dialog = this.$fixed.closest('.dialogShadow');
            	if (this.$container.hasClass('tab-pane')) {
            		dialogHeight = dialogHeight-$dialog.find('.bjui-pageHeader').height()-$dialog.find('.bjui-pageFooter').height()-118;
            	} else {
            		dialogHeight = dialogHeight-$dialog.find('.bjui-pageHeader').height()-$dialog.find('.bjui-pageFooter').height();
            	}                
                this.$fixed.height(dialogHeight).find('.fixedtableScroller').height(dialogHeight-50);
                //$dialog.find('.bjui-pageFooter').width(width);
            }else{
            	var $ntab = this.$fixed.closest('.unitBox');
            	height = $ntab.height()-$ntab.find('.bjui-pageHeader').height()-$ntab.find('.bjui-pageFooter').height()-10;
                this.$fixed.height(height).find('.fixedtableScroller').height(height - this.$fixed.find('.fixedtableHeader').height());
                $ntab.find('.bjui-pageFooter').width(width);
            }
        }  // zhilian.end
    }
    ...
    Tablefixed.prototype.initBody = function() {
        ...
        if (this.options.height) {
            ...
            var resizeH = function() {
            	// zhilian 原来的高度有bug               
                var dialogHeight = that.$fixed.closest('.dialogShadow').height();
                if (dialogHeight) {                	
                ...
                } else { // 增加 navtab 高度的计算
                	var $ntab = that.$fixed.closest('.unitBox');
                	var _height = $ntab.height()-$ntab.find('.bjui-pageHeader').height()-$ntab.find('.bjui-pageFooter').height()-10;
                	that.$fixed.height(_height).find('.fixedtableScroller').height(_height - that.$fixed.find('.fixedtableHeader').height());               	
                }
            }
            $(document).one(BJUI.eventType.afterInitUI, function(e) {
        ...
        // zhilian <div class="fixedtableTbody" 添加 style="overflow-x: hidden">
        $tbody.wrap('<div class="fixedtableScroller"'+ style +' style="width:'+ (this.options.newWidth) +'px;"><div class="fixedtableTbody" style="overflow-x: hidden"><table style="width:'+ (this.options.newWidth - Tablefixed.SCROLLW) +'px; max-width:'+ (this.options.newWidth - Tablefixed.SCROLLW) +'px;"></table></div></div>');
    ...
    // zhilian begin 双击打开 dialog
    $(document).on('dblclick.bjui.tr.data-api', 'tr[data-id]', function(e) {
        var $this = $(this);
        var dbDialog = $this.attr("dbDialog") || false;
        if (dbDialog) {
            var title = $this.attr("dbTitle") || "New Dialog";
            var dbDialogId = $this.attr("data-id") || "_blank";
            var options = {};
            options.title = title;
            var w = $this.attr("dbWidth");
            var h = $this.attr("dbHeight");
            ...
            if (!url.isFinishedTm()) {
                ...
                //$.pdialog.open(url, dbDialogId, title, options);
                $this.dialog({id:dbDialogId || 'viewdialog', url:url, title:options.title, width:options.width, height:options.height, mask:options.mask, maxable:options.maxable, resizable:options.resizable})
            }
        }
    });
    // zhilian end
    ...
    Tabledit.prototype.init = function() {
        ...
        $tr.find('> th').each(function() {
            ...
            if (add) {
                ...
                $th.empty();
                that.$numAdd = $('<input type="text" value="1" class="form-control num-add" size="4" title="待添加的行数"> ');
                that.$btnAdd = $('<a href="javascript:;" class="row-add" title="添加行" style="padding-top:-5px;"><i class="fa fa-plus-square"></i></a>');
...
+function ($) {
    ...
    Lookup.prototype.setMult = function(id,type) {
        ...
        if ($.isEmptyObject(args)) {
            ...
            // return
            // zhilian begin
            if (this.$element.data('warn')) {
                this.$element.alertmsg('error', this.$element.data('warn') || BJUI.regional.selectmsg)
                return
            } else {
                $unitBox.find('[name="'+ id +'"]').first().each(function() {
                    var _args = new Function('return '+ $(this).val())();
                    for (var key in _args) {
                        args[key] = '';
                    }
    ...
    $(document).on('click.bjui.lookup.data-api', '[data-toggle="lookupbtn"]', function(e) {
        ...
        if ($this.data("catch")) {
            ...
            if  ($this.data('url')) href = $this.data('url');
            var catchField = $this.data('catch');
            var catchValue = $("#"+catchField).val();
            href = href + "-"+catchValue;
            $this.data('url', href);
        } else {
            if ($this.attr('href') && !$this.data('url')) $this.attr('data-url', $this.attr('href'));
        }
...
+function ($) {
    ...
    Upload.prototype.init = function() {
        ...
        var $uploadFrag = $(FRAG.uploadFrag
            ...
            .replaceAll('#btnTxt#', (options.icon ? '<i class="fa fa-'+ options.icon +'" style="margin-top:3px;">&nbsp;&nbsp;' : '') + options.buttonText))

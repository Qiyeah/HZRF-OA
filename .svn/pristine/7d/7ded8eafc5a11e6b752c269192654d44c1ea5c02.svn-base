...
<link type="text/css" rel="stylesheet" href="css/calendarAll.css" />
                ...
                <TBODY>
                    <TR>
                        <TD class="calTit" colSpan=7 style="height: 30px; padding-top: 3px; text-align: center;">
                            <!-- <a href="javascript:;" title="上一年" id="nianjian" class="ymNaviBtn lsArrow"></a> --> <a href="javascript:;" title="上一月" id="yuejian" class="ymNaviBtn lArrow"></a>

                            <div style="float: left; padding-left: 230px;">

                                <span id="dateSelectionRili" class="dateSelectionRili" style="cursor: hand; color: white; border-bottom: 1px solid white;" onclick="dateSelection.show()"> <span
                                    id="nian" class="topDateFont"></span><span class="topDateFont">年</span><span id="yue" class="topDateFont"></span><span class="topDateFont">月</span> <span
                                    class="dateSelectionBtn cal_next" onclick="dateSelection.show()">▼</span></span> &nbsp;&nbsp;<font id=GZ class="topDateFont"></font>
                                ...
                                <div id="dateSelectionBody">
                                    ...
                                    <div id="monthList">
                                        <div id="monthListContent">
                                            <span id="SM0" class="month" onclick="dateSelection.setMonth(0)">1</span><span id="SM1" class="month" onclick="dateSelection.setMonth(1)">2</span><span id="SM2"
                                                class="month" onclick="dateSelection.setMonth(2)">3</span><span id="SM3" class="month" onclick="dateSelection.setMonth(3)">4</span><span id="SM4" class="month"
                                                onclick="dateSelection.setMonth(4)">5</span><span id="SM5" class="month" onclick="dateSelection.setMonth(5)">6</span><span id="SM6" class="month"
                                                onclick="dateSelection.setMonth(6)">7</span><span id="SM7" class="month" onclick="dateSelection.setMonth(7)">8</span><span id="SM8" class="month"
                                                onclick="dateSelection.setMonth(8)">9</span><span id="SM9" class="month" onclick="dateSelection.setMonth(9)">10</span><span id="SM10" class="month"
                                                onclick="dateSelection.setMonth(10)">11</span><span id="SM11" class="month curr" onclick="dateSelection.setMonth(11)">12</span>
                                        </div>
                            ...
                            <button type="button" class="btn-green" onclick="buttonWork()">选中工作日</button> <!-- <a href="javascript:;" id="nianjia" title="下一年" class="ymNaviBtn rsArrow" style="float:right;"></a> -->
                            <a href="javascript:;" id="yuejia" title="下一月" class="ymNaviBtn rArrow" style="float: right;"></a>
                        </TD>
                    </TR>
                    ...
                    <c:forEach var="i" begin="0" end="5">
                        <tr style="table-layout: fixed" align=center height="50" id="tt">
                        <c:forEach var="j" begin="0" end="6">
                            <c:set var="gNum" value="${i * 7 + j }" />
	                        <td id="GD${gNum}" on="0">
	                           <span id="SD${gNum}" style="font-size: 22px;font-family:Arial;<c:if test="${j==0 || j==6}">color:red</c:if>"></span><br> 
	                           <font id="LD${gNum}" size="2" style="white-space: nowrap; overflow: hidden; cursor: default;"> </font></td> 
                        </c:forEach>
                        </tr>
                    </c:forEach> 
                </tbody>
            </TABLE>
            <table class="biao" style="width: 800px; margin-top: 10px;">
                <tr>
                    <td>
                        <div id="workday" class="" style="height: 100px;; overflow-y: scroll;">
                            <p id="setworkday">
                                <span>选定工作日</span>:
                            </p>
                        </div>
...
$("td").click(function(){    //td点击事件；
    ...
    //提交
function h_submit(flag){
    var chkValue =[];
    $('input[name="workingDay.date"]').each(function(data){  
    chkValue.push($(this).val());    
    }); 
    if(chkValue == ''){
    $(this).alertmsg('error', '请选择日期！');
    return false;
    }else{
        $('input[name="workingDays"]').val(chkValue.join(","));
        $('input[name="flag"]').val(flag);
        $("#WorkingDayForm").submit();
    }
    return true;
}
  //重置
...
function drawCld(SY, SM) {
    ...
    $("#GZ")[0].innerHTML = '  农历' + cyclical(SY - 1900 + 36) + '年&nbsp;【' + Animals[(SY - 4) % 12] + '年】';
        $('#setworkday').html("<span>选定工作日</span>");
        $('input[name="workingDays"]').val(''); 
    for (i = 0; i < 42; i++) {
        sObj = $("#SD" + i)[0];
        $("#GD"+i).attr("on",'0');
        ...
        if (sD > -1 && sD < cld.length) {  //日期内
            ...
            var nowDays1 = SY+'-'+addZ((SM+1))+'-'+addZ((sD+1));
 ...
 function buttonWork(){
 var tY = global.currYear; var tM = global.currMonth;
    ...
    sD = i - cld.firstWeek;
    var nowDaysId = tY+''+addZ((tM+1))+''+addZ((sD+1));
    if($('#'+nowDaysId).text() != ''){
        continue;
    }
    
    
    var nowDays1 = tY+'-'+addZ((tM+1))+'-'+addZ((sD+1));
    var dayss = new Date(''+nowDays1).getDay();
     $("#GD"+i).attr("on",'0');
    if( dayss > 0 && dayss < 6){
...
function mOck(thisObj, v,i){
    ...
    var ma = '-';
    var ymd1 = nian + ma +  addZ(yue) +ma+ addZ(day);
    ...
    //var sssss=document.getElementById(ymd).value;<img id='imgchlik' src='../img/false.png'/>加×号的
        ...
        if (onoff == "0") {
            ...
            document.getElementById("setworkday").innerHTML += "<span  class='date' id="+ ymd +">"+ymd1+" <input type='hidden' name='workingDay.date' value='"+ymd1+"'>   "+"<img id='"+ymd+"' value='"+ymd+"' onClick='imgonclick1("+ymd+","+ids+","+lx+")' src='images/false.png'/></span>";//将内容加入框中
 ...
 * 图片点击X事件
 */
function imgonclick1(value,ids,lx){
...
$(function() {
    ...
    $("#yuejian").click(function() {    
        $(this).alertmsg("confirm", "该操作保存当前数据，进入上一个月操作！",{
            okCall: function() {
                h_submit(0);
    ...
    $("#yuejia").click(function() {        
        $(this).alertmsg("confirm", "该操作保存当前数据，进入下一个月操作！",{
            okCall: function() {
                h_submit(0);
    ...
    <form id="WorkingDayForm" method="post" action="Main/WorkingDay/add" data-toggle="validate" data-callback="approveCallback">
        <input type="hidden" name="workingDays"> <input type="hidden" name="flag">
    </form>
...
<div class="bjui-pageFooter">
    <ul>
        ...
        <li><button type="button" class="btn-default" onclick="h_submit(1)">保存</button></li>
    </ul>
</div>

		...
		<div class="pageFormContent">
			...
			<table class="wordInfo" align="center" style="width: 95%;margin-bottom:30px">
				<tr>
					...
					<td width="17%"><input name="t_Doc_Receive.receivedate" data-toggle="datepicker" type="text" value="${dc.receivedate}" class="required date" data-rule="required" style="width: 100%" /></td>
				...
				<tr>
					...
					<td colspan="2"><select name="t_Doc_Receive.security" data-style="selectrequired" data-msg-required="必填" data-rule="required" data-toggle="selectpicker" data-width="100%">
					...
					<td class="title">督办员</td>
					<td>
						<select name="t_Doc_Receive.superman" data-toggle="selectpicker" data-width="100%">
						    <option value="" >---请选择---</option>
						    <c:forEach items="${offmebers}" var="off">
							<option value="${off.id}" <c:if test="${dc.superman==off.id}">selected</c:if>>${off.name}</option>
							</c:forEach>
						</select>
					</td>
				</tr>
				<tr>
					...
					<td colspan="6" class="normal" align="center"><input name="t_Doc_Receive.title" type="text" value="${dc.title}" data-rule="required" class="required" 
				...
				<tr>
					...
					<td colspan="6"><div id="uploadFile" class="unitBox"><c:import url="../../Common/Attachment/edit.jsp" /></div></td>
				</tr>
	            <tr>
				...
				<tr>
					...
					<c:if test="${wa.haveopinionfield == '1' && wa.opinionfield == 'opinion1' && superdoflag !='2' && wf.isreceive=='0'}">
					    ...
					    <c:if test="${todomannum > 1}"> 
					          <c:if test="${wa.amount != 3 &&  wf.itemid == '26'}">
					             <a class="btn btn-blue" data-icon="fa-check" onclick="ChuLiWanBi()" style="margin-left: 5px;">提交</a>
					          </c:if>
					          <c:if test="${wa.amount == 3 || (wf.iscanreceive =='1' && wf.itemid != '26')  && mustreceive=='0' &&  (nextstepsnum>0 || wf.isreceive=='1'|| wf.iscanreceive=='0')}">
				...
				<tr>
					...
					<c:if test="${wa.atype != '3'}">
						...
						<c:if test="${wa.amount == 3 || (wf.iscanreceive =='1' && wf.itemid != '26' && wf.itemid != '26') && mustreceive=='0' &&  (nextstepsnum>0 || wf.isreceive=='1'|| wf.iscanreceive=='0')}">
				...
				<tr><td class="title" style="height: 150px;"><br>办<br><br>理<br><br>情<br><br>况<br>
				<td colspan="6"><div id="opinion3_1">${opinion3}</div>
				<c:if test="${isreceive=='1' && mustreceive=='0' }">
				<a class="btn btn-blue" data-icon="fa-edit" onclick="editopinion()" style="margin-left: 5px;">填写意见</a>
				<c:if test="${todomannum > 1}">
					          <c:if test="${wa.amount != 3 && (wf.itemid == '26' || wf.itemid == '24')}">
					             <a class="btn btn-blue" data-icon="fa-check" onclick="ChuLiWanBi()" style="margin-left: 5px;">提交</a>
				        ...
				        <c:if test="${todomannum <= 1}">
					      <c:if test="${wa.atype != '3'&&(nextstepsnum>0 || isreceive=='1'|| wf.iscanreceive=='0')&& mustreceive=='0'}">
						      <a class="btn btn-blue" data-icon="fa-check" onclick="FaSongdoc()" style="margin-left: 5px;">提交</a>
					      </c:if>
				          </c:if>
				</c:if></td>
			...
			<input type="hidden" name="isreceive1" id="isreceive1" value="${isreceive }">
...
<div id="bar" class="bjui-pageFooter">
	<ul>
		...
		<c:if test="${superdoflag == '3'}">
			<li><a class="btn btn-blue" data-icon="fa-rotate-right" onclick="TeSong()">重置处理人</a></li>
		</c:if>
		<c:if test="${superdoflag != '2' && superdoflag != '3'}">
			...
			<c:if test="${candeal}">
				...
				<c:if test="${wa.id==107 && mustsubmit=='0'}"><!-- 除了已签收的人员-->
				...
				<c:if test="${todomannum <= 1}">
					...
					<c:if test="${wa.atype != '3'&& (nextstepsnum>0 || isreceive=='1'|| wf.iscanreceive=='0')}">
						<li><a class="btn btn-blue" data-icon="fa-check" onclick="FaSongdoc()">提交</a></li>
					</c:if>
				</c:if>

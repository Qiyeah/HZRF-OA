...
<script type="text/javascript">
	$("input[name=selectwas]").click(function() {
		...
		$("#nextitemid", window.parent.document).val(itemid);
	...
	$("#users").ajaxUrl({ url : "Main/Workflow/selectman2/" + itemid + "-${cond}", loadingmask : true });
	}   */
	...
	function send() {
		...
		/* 		$("#users").find("input[type='checkbox']").each(function(i) {
		
		 if (this.checked && this.name != 'dept') { 
		 var bool = tmpstr.indexOf($(this).val());
		 if (bool < 0) {
		 tmpstr += ";" + $(this).val();
		 tmpnum = tmpnum + 1;
		 }

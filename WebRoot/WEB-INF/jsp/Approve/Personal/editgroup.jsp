...
<div class="bjui-pageContent">
		<form id="pageForm1" class="pageForm" action="Main/Personal/editgroup" method="post" data-toggle="validate">		
		<input name="t_Group.id" class="required" type="hidden" value="${group.id}" />
		<label style="margin-bottom:10px">群组名称:</label><span><input type="text" name="t_Group.name" value="${group.name }" class="required" data-rule="required"></span>
			...
			<table class="table table-bordered table-hover table-striped table-top table-center">
				...
				<tbody id="resultBox">
					<c:forEach items="${checkUsers }" var="checkUser">
						<tr><td><input type="hidden"  name="u_ids" value="${checkuser.id }"/><input type="hidden"  name="u_names" value="${checkuser.name }"/>${checkUser.name }</td></tr>
					</c:forEach>
				</tbody>
			</table>

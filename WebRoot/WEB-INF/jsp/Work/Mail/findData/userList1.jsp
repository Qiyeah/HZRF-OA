...
<div class="bjui-pageContent">
	<div class="pageFormContent">
		...
		<div style="float:left; display:block; overflow:auto; width:280px; height:100%; border:solid 1px #4CA5D8;">
			<ul id="urse-tree" class="ztree" data-toggle="ztree" data-expand-all="false" data-check-enable="true" data-on-check="onCheck">
				...
				<c:if test="${! empty positionList}">
					<c:forEach items="${positionList}" var="position">
						<li data-id="${position.id}" data-pid="${position.pid}">${position.name}</li>
					</c:forEach>
					<c:forEach items="${userList}" var="user">
						<li data-id="10000" data-faicon="user" <c:forEach items="${checkUsers}" var="checkUser"><c:if test="${checkUser.id==user.id }">data-checked="true"</c:if></c:forEach> 
						data-value="${user.id}" data-name="${user.name}" data-pid="${user.pid }">${user.name} (${deptHM.get(user.d_id) })</li>
					</c:forEach>
				</c:if>
			</ul>
